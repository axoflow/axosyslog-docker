...
_save_addresses(LogTransportHAProxy *self)
{
  ...
  if (self->info.ip_version == 4)
    {
      log_transport_aux_data_set_peer_addr_ref(&stack->aux_data, g_sockaddr_inet_new(self->info.src_ip, self->info.src_port));
      log_transport_aux_data_set_local_addr_ref(&stack->aux_data, g_sockaddr_inet_new(self->info.dst_ip, self->info.dst_port));
    }
  else if (self->info.ip_version == 6)
    {
      log_transport_aux_data_set_peer_addr_ref(&stack->aux_data, g_sockaddr_inet6_new(self->info.src_ip, self->info.src_port));
      log_transport_aux_data_set_local_addr_ref(&stack->aux_data, g_sockaddr_inet6_new(self->info.dst_ip, self->info.dst_port));
    }
  else
    g_assert_not_reached();
}
